...
def extract_license_info(project_url, personal_token_key):
    ...
    response = requests.get(
        f"{gitlab_base_url}/projects/{project_id}/repository/files/LICENSE.txt/raw?ref=master", timeout=10)
    ...
    if response.status_code == 404:
        response = requests.get(
            f"{gitlab_base_url}/projects/{project_id}/repository/files/LICENSE/raw?ref=master", timeout=10)
        if response.status_code == 200:
            license_name = None
            license_text = response.text.strip()
            license_name = license_text.splitlines()[0]
